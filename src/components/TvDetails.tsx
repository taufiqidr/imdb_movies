...
import type { Credits, TVDetails } from "types/types";

const TvDetails = () => {
  const router = useRouter();
  const { id } = router.query;

  const fetchTvDetails = async (): Promise<TVDetails> => {
  ...
  const fetchTvCredits = async (): Promise<Credits[]> => {
    const apiKey = process.env.NEXT_PUBLIC_TMDB_API_KEY;
    const response = await axios.get(
  ...
  const {
    ...
    error: errorCredits,
  } = useQuery<Credits[], Error>(["credits", id], fetchTvCredits, {
    enabled: !!id,
  });
